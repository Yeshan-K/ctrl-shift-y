...
#### yjs-orderedtree: _Y.Maps and algorithms in a trenchcoat_
...
* A Y.Map that holds parent history, with the parent node ID as a key and an object containing:
  ...
  * `order_index`: A fractional number (stored as a string) to resolve sibling ordering.
...
That still doesn't solve the second situation, and if you are fan of bogo sort like then you will definitely like how this second situation is solved. One word and five syllables, you guessed it; Randomization. You randomize the order index of node C being inserted between node A and B to be any fractional number between the order indexes of A and B.
...
```javascript
node.parentHistory = {
    "parentId1" : {position: 0, order_index: 0.75026},
    "parentId2" : {position: 1, order_index: 0.25002},
    .
    .
    . and so on
    .
...
* "parentHistory" :  value is a Y.Map(
  * ```javascript
    {   "parentId1" : {position: 0, order\_index: 0.75026},
        "parentId2" : {position: 1, order\_index: 0.25002},
        .
        .
        . and so on
        .
...
Go ahead, pause and unpause synchronization and create/move around nodes as you see fit. Try creating cycles as well.
